...
- **Penjualan & pencapaian**: `src/lib/achievements.ts` — **localStorage browser** (keys `komoditasumut:sales` / `komoditasumut:achievements`), API identik dengan versi Postgres lama (recordSale, getSellerStats, getPlatformAnalytics). Cross-tab sync via event `komoditasumut:sales-changed`
- ⚠️ Per-browser & per-device; clear site data menghapus data. Kalau butuh multi-device nanti: balikin storage layer ke Postgres hanya dengan mengubah file ini
...
| Env vars | Auth-only (NEXTAUTH, Google, GitHub) — tidak ada DB env |
